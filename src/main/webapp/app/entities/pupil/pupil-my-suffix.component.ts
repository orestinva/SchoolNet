...
@Component({
    selector: 'jhi-pupil-my-suffix',
    templateUrl: './pupil-my-suffix.component.html'
})
export class PupilMySuffixComponent implements OnInit, OnDestroy {
pupils: PupilMySuffix[];
    ...
    trackId(index: number, item: PupilMySuffix) {
        return item.id;
    }
    registerChangeInPupils() {
        this.eventSubscriber = this.eventManager.subscribe('pupilListModification', (response) => this.loadAll());
    }
